...
export default {
  fetch: withSupabase({ auth: 'publishable' }, async (req, ctx) => {
    ...
    // Log helper
    const logAudit = async (action: string, email: string | null, status: string) => {
      await ctx.supabaseAdmin.from('security_audit_log').insert({
        ip_address: clientIp,
        action,
        email,
        status
    ...
    try {
      ...
      if (rlData) {
        ...
        if (now - windowStart < 10 * 60 * 1000) {
          if (rlData.attempt_count >= 5) {
            ...
            return Response.json({ error: 'Too many signup attempts. Please try again later.' }, { status: 429, headers: corsHeaders });
          }
          // increment
          await ctx.supabaseAdmin.from('signup_rate_limit')
        ...
        await ctx.supabaseAdmin.from('signup_rate_limit').insert({ ip_address: clientIp, attempt_count: 1 });
      }
      ...
      if (!parseResult.success) {
        ...
        return Response.json({ error: 'Invalid input format' }, { status: 400, headers: corsHeaders });
      }

      const { type, email, password, display_name, phone, region, technical_skill, vehicle_available, captcha_token } = parseResult.data;

      // 3. CAPTCHA Verification
      const captchaValid = await verifyTurnstile(captcha_token, clientIp);
      if (!captchaValid) {
        await logAudit('signup_captcha', email, 'failed');
        return Response.json({ error: 'Security check failed. Please try again.' }, { status: 400, headers: corsHeaders });
      }
