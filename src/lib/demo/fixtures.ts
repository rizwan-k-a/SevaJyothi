...
export function isDemoMode(): boolean {
  ...
  if (new URLSearchParams(window.location.search).get("demo") === "true") {
    try {
      sessionStorage.setItem("sj-demo", "1");
    } catch {
      /* ignore */
    }
    return true;
  ...
  try {
    return sessionStorage.getItem("sj-demo") === "1";
  } catch {
    return false;
  }
}
...
export function resetDemoMode(): void {
  ...
  try {
    sessionStorage.removeItem("sj-demo");
  } catch {
    /* ignore */
  }
...
export const DEMO_COMPLAINTS: DemoRow[] = [
  {
    ...
    accuracy: 18,
    village: "Anekal",
    reporter_id: "demo-citizen-1",
    assigned_to: "demo-tech-1",
  },
  {
    id: "demo-2",
    client_id: "demo-2",
    category: "network_tower",
    ...
    accuracy: 42,
    village: "Sarjapur",
    reporter_id: "demo-citizen-2",
    assigned_to: "demo-tech-2",
  },
  {
    id: "demo-3",
    client_id: "demo-3",
...
export const DEMO_STATS = {
  ...
  critical: DEMO_COMPLAINTS.filter(
    (c) => c.priority === "critical" && !["resolved", "closed"].includes(c.status),
  ).length,
  last24h: DEMO_COMPLAINTS.filter((c) => Date.now() - new Date(c.created_at).getTime() < 86_400_000)
    .length,
