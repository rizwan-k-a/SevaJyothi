...
function OfflineDoc() {
  return (
    <article className="mx-auto max-w-3xl px-6 pt-32 pb-24">
      ...
      <div className="text-mono-data text-[11px] uppercase tracking-[0.16em] text-accent">Architecture</div>
      <h1 className="mt-2 text-display text-[clamp(2rem,4vw,3rem)]">Offline-first by construction</h1>
      <p className="mt-3 text-[15px] text-muted-foreground">
        Rural India does not have continuous LTE. SevaJyothi never assumes the network is there — every write commits locally first, then propagates.
      </p>

      <div className="mt-10 grid gap-4">
        <Block icon={Database} title="Local store · IndexedDB"
        ...
        <Block icon={CloudOff} title="Capture without signal"
        ...
        <Block icon={RefreshCcw} title="Automatic replay"
        ...
        <Block icon={ShieldCheck} title="No data loss invariant"
      ...
      <div className="mt-10 glass rounded-3xl p-6 text-[13.5px] text-muted-foreground">
        <div className="text-mono-data text-[11px] uppercase tracking-[0.14em] text-accent">Implementation</div>
        <ul className="mt-3 list-disc space-y-1 pl-5">
          <li><code className="text-mono-data">src/lib/offline/db.ts</code> — IDB CRUD and Supabase sync bridge</li>
          <li><code className="text-mono-data">src/components/providers/OfflineProvider.tsx</code> — online listener + SW message bridge</li>
          <li><code className="text-mono-data">public/sw.js</code> — Service Worker with Background Sync tag <code>sj-flush-complaints</code></li>
        </ul>
      </div>
    </article>
  );
