...
const ComplaintMap = lazy(() =>
  import("@/components/map/ComplaintMap")
    ...
    .catch((err) => {
      ...
      // Return a fallback stub so the rest of the app doesn't crash
      return {
...
function CitizenMiniMap(p: {
  ...
  return (
    <Suspense
      fallback={
        <div className="grid h-[180px] place-items-center rounded-2xl bg-muted/30 text-[11px] text-muted-foreground">
          Loading map…
        </div>
      }
    >
...
function CitizenDashboard() {
  ...
  useEffect(() => {
    loadRemote(); /* eslint-disable-next-line */
  ...
  const inProgress = remote.filter(
    (c) => !["submitted", "resolved", "closed"].includes(c.status),
  ).length;
  const submitted = remote.filter((c) => c.status === "submitted").length;

  return (
    <div
      className="mx-auto max-w-6xl px-4 pt-20 pb-28 sm:px-6 sm:pt-32 sm:pb-24"
      style={{
    ...
    >
      <div className="mb-8 flex flex-wrap items-end justify-between gap-4 sm:mb-10 sm:gap-6">
        <div className="min-w-0">
          ...
          <p className="mt-2 max-w-xl text-[14px] text-muted-foreground sm:text-[14.5px]">
            Report a new failure, or check the status of what you've already raised. Everything you
            log here is safe — even without signal.
          </p>
        </div>
      ...
      >
        <div>
          ...
          <div className="mt-1 text-[14px] text-muted-foreground">
            Photo · GPS · Category · Sync. Two taps.
          </div>
        </div>
        ...
        <section className="mb-10">
          <div className="mb-4 flex items-center justify-between">
            ...
            <button
              ...
              className="text-mono-data text-[11px] uppercase tracking-[0.14em] text-accent hover:underline"
              data-cursor="link"
            >
              Retry sync
            </button>
          ...
          <div className="glass rounded-3xl p-2">
            {items.unsynced.map((c) => (
              <LocalRow key={c.id} c={c} />
            ))}
          </div>
          <p className="mt-3 px-2 text-[12.5px] text-muted-foreground">
            <span className="text-foreground">Stored securely in IndexedDB.</span>{" "}
            {online ? "Reconnecting now…" : "Waiting for signal. We'll sync automatically."}
          </p>
        </section>
      )}
          ...
          {loadingRemote ? (
            ...
            <div className="px-6 py-12 text-center">
              ...
              <div className="text-[14px] text-muted-foreground">
                No reports yet — your queue is clear.
              </div>
            </div>
          ) : (
      ...
      >
        ...
        <span className="text-mono-data text-[11px] uppercase tracking-[0.16em] opacity-80">
          Two taps
        </span>
      </Link>
    </div>
  ...
  return (
    ...
    >
      <span className="relative inline-flex h-2.5 w-2.5">
        <span
          className="absolute inset-0 animate-sj-pulse-ring rounded-full"
          style={{
            background: online ? "oklch(0.70 0.16 162)" : "oklch(0.62 0.23 27)",
            opacity: 0.6,
          }}
        />
        <span
    ...
    >
      <div className="flex items-start justify-between">
        <div>
          ...
          <div className="mt-2 text-mono-data text-[32px] leading-none text-ink">
...
function LocalRow({ c }: { c: StoredComplaint }) {
  ...
  return (
    <div
      className="group flex items-center gap-4 rounded-2xl p-4 transition hover:bg-white/60"
      data-cursor="card"
    >
      <div
...
function StatusPill({ label, tone }: { label: string; tone: "warn" | "info" | "progress" | "ok" }) {
  ...
  return (
    <span
      className="inline-flex items-center rounded-full px-2 py-0.5 text-mono-data text-[10px] uppercase tracking-[0.12em]"
