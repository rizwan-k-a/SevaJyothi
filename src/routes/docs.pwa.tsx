  ...
  return (
    <article className="mx-auto max-w-3xl px-6 pt-32 pb-24">
      ...
      <p className="mt-3 text-[15px] text-muted-foreground">
        SevaJyothi ships as a Progressive Web App — no Play Store, no App Store, no waiting. The
        same URL becomes an installed app on every device.
      </p>
      ...
      <div className="mt-10 grid gap-4 sm:grid-cols-2">
        <Steps title="Android · Chrome">
          ...
          <li>
            Tap the ⋮ menu → <span className="text-foreground">Add to Home screen</span>.
          </li>
        ...
        <Steps title="iOS · Safari">
          ...
          <li>
            Tap the share button → <span className="text-foreground">Add to Home Screen</span>.
          </li>
...
function Steps({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div className="glass rounded-2xl p-6">
      ...
      <ol className="mt-3 list-decimal space-y-1.5 pl-5 text-[13.5px] text-muted-foreground">
        {children}
      </ol>
    </div>
  );
