...
envContent.split('\n').forEach(line => {
  if (line.trim() && !line.startsWith('#')) {
    ...
    if (key) env[key.trim()] = val.join('=').trim().replace(/^["'](.*)["']$/, '$1');
  }
});
...
async function runTests() {
  ...
  const { data: adminCheck, error: adminErr } = await adminClient.from('profiles').select('id').limit(1);
  if (adminErr) {
  ...
  const { data: bucketData, error: bucketErr } = await adminClient.storage.getBucket('complaint-media');
  if (bucketErr) {
    ...
    const { error: uploadErr } = await adminClient.storage.from('complaint-media').upload(fileName, testBuffer);
    if (uploadErr) {
      console.log("❌ Image Upload Failed:", uploadErr.message);
    } else {
      console.log("✅ Image Upload Passed");
      const { data: signedUrlData, error: urlErr } = await adminClient.storage.from('complaint-media').createSignedUrl(fileName, 60);
      if (urlErr) {
  ...
  if (adminLoginErr) {
    ...
    const { data: adminRole } = await anonClient.from('user_roles').select('role').eq('user_id', adminLogin.user.id).single();
    if (adminRole?.role === 'authority') {
      console.log("✅ Admin Login Passed. Role verified as 'authority'.");
    } else {
      console.log("❌ Admin Login Passed, but role is NOT 'authority'. Current role:", adminRole?.role);
    }
  }
  ...
  console.log(hasPushSw ? "✅ Service Worker (sw.js / push-sw.js) exists." : "❌ Service Worker missing.");
  ...
  // Verify table
  const { error: pushTableErr } = await adminClient.from('push_subscriptions').select('id').limit(1);
  if (pushTableErr) {
