...
async function runTests() {
  ...
  try {
    ...
    if (loginErr) {
      ...
      log(
        "PHASE 3 - LOGIN",
        `Success. JWT returned: ${!!loginData.session.access_token}`,
        "WORKING",
      );
    }
    ...
    const { data: uploadData, error: uploadErr } = await supabase.storage
      ...
      .upload(filePath, dummyContent, { upsert: true });
    ...
    if (uploadErr) {
      ...
      if (signedUrlErr) {
        ...
        log(
          "PHASE 7 - STORAGE",
          `Upload & Signed URL success. URL: ${signedUrlData.signedUrl.substring(0, 50)}...`,
          "WORKING",
        );
      }
    }
